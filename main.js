...
function convertCombinedShaderToWebGL(source, vertexStage) {
  const stageSource = extractShaderStage(source, vertexStage)
...
function compileShader(glCtx, type, source) {
  const compiledShader = glCtx.createShader(type);
  glCtx.shaderSource(compiledShader, source);
  glCtx.compileShader(compiledShader);
  if (!glCtx.getShaderParameter(compiledShader, glCtx.COMPILE_STATUS)) {
    const log = glCtx.getShaderInfoLog(compiledShader);
    glCtx.deleteShader(compiledShader);
